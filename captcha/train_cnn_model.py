...
def cnn_model(input_shape, model_name='Captcha-Recognition-Model'):
    """
    keras实现的cnn模型
    """
...
def train_cnn(X_train, Y_train, X_test, Y_test, epochs=10, batch_size=64, model_view=MODEL_VIEW):
    ...
    model.fit(x=X_train, y=Y_train, validation_data=(X_test, Y_test), epochs=epochs, batch_size=batch_size, callbacks=[tb])

    # Save the trained model to disk
    model.save(os.path.expanduser(MODEL_FILENAME))
    ...
    plot_model(model, to_file=os.path.expanduser(model_view))
